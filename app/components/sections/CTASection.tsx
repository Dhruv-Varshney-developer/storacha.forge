            ...
            {steps.map((step) => (
              <div key={step.number} className="flex items-center gap-3">
                <div className="bg-[#E91315] border-2 border-white rounded-full w-8 h-8 flex items-center justify-center">
                  <span className="font-epilogue font-medium text-white text-lg">{step.number}</span>
                </div>
                <p className="font-dm-sans font-medium text-xl text-white">{step.text}</p>
              </div>
            ))}
          </div>
          
          <button className="bg-[#E91315] text-white px-8 py-5 rounded-full font-semibold text-2xl flex items-center gap-2.5 hover:bg-red-700 transition-colors">
